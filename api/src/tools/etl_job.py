...
def run_etl():
    ...
    if activities:
        ...
        new_activities = [a for a in activities if not last_act_date or pd.to_datetime(a.date).tz_localize(None) > last_act_date.tz_localize(None)]
        
        if new_activities:
            df_act = pd.DataFrame([a.model_dump() for a in new_activities])
            if 'splits' in df_act.columns: df_act.drop(columns=['splits'], inplace=True)
