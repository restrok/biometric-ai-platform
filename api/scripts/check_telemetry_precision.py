...
def audit_telemetry_precision(
    ...
    if json_path:
        ...
        log.info(
            f"Fetching raw data from BigQuery for activity: {activity_id or 'latest'}"
        )
    ...
    if pwr_col and cad_col:
        raw_15s["is_work"] = (
            (raw_15s[pwr_col] > 180) | (raw_15s[cad_col] > 145)
        ).astype(int)
    else:
    ...
    raw_15s["change"] = (
        (raw_15s["is_work"].diff().fillna(0).abs() > 0)
        | (raw_15s["time_bucket"].diff().fillna(0).abs() > 0)
    ).astype(int)
    ...
    final_segments.columns = [
        c[0] if c[1] in ["", "first", "mean"] else f"{c[0]}_{c[1]}"
        for c in final_segments.columns
    ]
    final_segments["dur"] = (
    ...
    for _, seg in final_segments[final_segments["dur"] >= 10].iterrows():
        mask = (df_raw["dt"] >= seg["time_block_15s_min"]) & (
            df_raw["dt"]
            <= (seg["time_block_15s_max"] + pd.Timedelta(seconds=14.9))
        )
        gt = df_raw[mask]
        ...
        for c in ["power_w", "hr_bpm", "body_battery", "stride_length_mm"]:
            if c in gt.columns:
                ...
                val_raw = gt[c].mean()
                drift = (
                    ((val_agg - val_raw) / val_raw * 100) if val_raw != 0 else 0
                )
                log.info(
                    f"  - {c:<18}: {val_agg:>7.1f} | {val_raw:>7.1f} | {drift:>6.2f}%"
                )

        # Check MAXes
        if "power_w_max" in seg:
            log.info(
                f"  - POWER MAX         : {seg['power_w_max']:>7.1f} | "
                f"{gt['power_w'].max():>7.1f} | 0.00%"
            )
        if "hr_bpm_max" in seg:
            log.info(
                f"  - HR MAX            : {seg['hr_bpm_max']:>7.1f} | "
                f"{gt['hr_bpm'].max():>7.1f} | 0.00%"
            )
