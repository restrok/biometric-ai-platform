...
*   **Repo 2: The AI Platform (Future, e.g., `biometric-ai-platform`)**
    ...
    *   **Interaction:** Consumes the structured data generated by Repo 1.

## 2. Refactoring the Current Repository (The Toolkit SDK)

To make the current repository useful for Repo 2's future Data Pipelines, we must transform it from "loose scripts" into a formal, strongly-typed Python library.

### A. Python Package Structure
We will consolidate existing code into a structured package:

```text
garmin-training-toolkit/
├── garmin_toolkit/               # The core package
│   ├── __init__.py
│   ├── auth.py                   # Login and token management
│   ├── client.py                 # Garmin API wrapper
│   ├── extractors/               # Domain-specific modules
│   │   ├── activities.py         # Training download (.fit / json)
│   │   ├── biometrics.py         # Sleep, HRV, Weight
│   │   └── metrics.py            # Readiness, VO2Max
│   └── models/                   # Pydantic models (Data Contracts)
├── data-pipeline/                # (Optional, for local ingestion testing)
└── pyproject.toml                # Management with uv or poetry
```

### B. Data Contracts
The main problem with the current implementation is the use of schemaless dictionaries. 
*   **Action:** Pydantic models will be implemented to strictly validate every payload Garmin returns before delivering it to the Data Pipeline.
*   **Benefit:** Prevents corrupt schemas or silent changes in the Garmin API from breaking Vector Databases or LLM prompts.

### C. Logic Decoupling
*   **Action:** Separate *Extraction* logic from *Formatting/Presentation* logic.
*   **Benefit:** The SDK should only return clean Python objects. Current Markdown report generation will move to an isolated "Local Visualization" module that the AI pipeline will not need to import.

## 3. Cost-Zero Development Strategy (FinOps)
