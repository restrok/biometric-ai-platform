...
The **Biometric AI Platform** is an enterprise-grade, multi-agent sports physiology platform built on **LangGraph**, **FastAPI**, and a **Hybrid GCP Lakehouse (Firestore + BigQuery)**. 
...
The platform executes an event-driven, graph-based agent flow orchestrated by **LangGraph**. Requests are dynamically routed based on user intent and processed through parallel analysis, deep exploratory query loops, and safety validation.
...
## 2. Specialist Agent Mandates

| Agent | Icon | Role & Mandate |
| :--- | :---: | :--- |
